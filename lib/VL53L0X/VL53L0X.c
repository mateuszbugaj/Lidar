...
// mode.
uint8_t VL53L0X_init(struct VL53L0X* dev)
{
  // check model ID register (value specified in datasheet)
  if (VL53L0X_readReg(dev, IDENTIFICATION_MODEL_ID) != 0xEE) { return 1; }
  ...
  if (!VL53L0X_getSpadInfo(dev, &spad_count, &spad_type_is_aperture)) { return 2; }
 ...
 VL53L0X_setMeasurementTimingBudget(dev, dev->measurement_timing_budget_us);
  ...
  if (!VL53L0X_performSingleRefCalibration(dev, 0x40)) { return 3; }
  ...
  if (!VL53L0X_performSingleRefCalibration(dev, 0x00)) { return 4; }
...
void VL53L0X_writeReg(struct VL53L0X* dev, uint8_t reg, uint8_t value)
{
	...
	buf[0] = reg;
	buf[1] = value;
	dev->last_status = i2c_writeN(dev->i2c, dev->address, buf, 2);
}
...
void VL53L0X_writeReg16Bit(struct VL53L0X* dev, uint8_t reg, uint16_t value)
{
	...
	dev->last_status = i2c_writeN(dev->i2c, dev->address, buf, 3);
}
...
void VL53L0X_writeReg32Bit(struct VL53L0X* dev, uint8_t reg, uint32_t value)
{
	...
	dev->last_status = i2c_writeN(dev->i2c, dev->address, buf, 5);
}
...
{
  uint8_t value;
  i2c_writeN(dev->i2c, dev->address, &reg, 1);
  dev->last_status = i2c_readN(dev->i2c, dev->address, &value, 1);
  return value;
}
...
uint16_t VL53L0X_readReg16Bit(struct VL53L0X* dev, uint8_t reg)
{
  ...
  uint8_t buf[2];
  i2c_writeN(dev->i2c, dev->address, &reg, 1);
  dev->last_status = i2c_readN(dev->i2c, dev->address, buf, 2);
...
uint32_t VL53L0X_readReg32Bit(struct VL53L0X* dev, uint8_t reg)
{
  ...
  uint8_t buf[4];
  i2c_writeN(dev->i2c, dev->address, &reg, 1);
  dev->last_status = i2c_readN(dev->i2c, dev->address, buf, 4);
...
{
	i2c_writeN(dev->i2c, dev->address, &reg, 1);
	dev->last_status = i2c_writeN(dev->i2c, dev->address, src, count);
}
...
{
	i2c_writeN(dev->i2c, dev->address, &reg, 1);
	dev->last_status = i2c_readN(dev->i2c, dev->address, dst, count);
}
...
void VL53L0X_startTimeout(struct VL53L0X* dev){
	dev->timeout_start_ms = systick_get_value();
}

bool VL53L0X_checkTimeoutExpired(struct VL53L0X* dev){
	return (dev->io_timeout > 0 && (systick_get_value() - dev->timeout_start_ms) > dev->io_timeout);
}
